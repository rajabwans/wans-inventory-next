...
export default function Page() {
  return <Suspense fallback={null}><AddSupplierPage /></Suspense>
}
    ...
    <DashboardShell businessName="" role="">
      <div className="max-w-xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-bold">{editId ? "Edit Supplier" : "Add Supplier"}</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
          ...
          <div>
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Link href="/suppliers" className="px-5 py-2 bg-gray-100 text-gray-600 rounded-lg text-sm font-medium hover:bg-gray-200">Cancel</Link>
          </div>
        </form>
      </div>
    </DashboardShell>
